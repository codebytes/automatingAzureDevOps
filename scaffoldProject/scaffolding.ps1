...
if ($repoName) {
    $buid = $null
    if ($repoName -eq $projectName) {
        $repo = az repos list --org $org --project $projectID -o json | convertfrom-json
    }
    else {
        $repo = createRepo  -repoName $repoName -org $org -projectID $projectID
    }
    $repoID = $repo.id
    if ($null -ne $repoId) {
        if ($repoToImport) {
            ...
            $tempRepoPath = . Join-Path $PSScriptRoot tempRepo\
        ...
        if ($requiredReviewers -or $optionalReviewers -or $build) {
            $policiesSet = set_policies -org $org -projectName $projectID -repoId $repoID -branch 'master' -requiredApprovers $requiredReviewers -optionalApprovers $optionalReviewers -buildId $build.definition.id
            Write-Host "`nBranch policies set for master"
            $policiesSet | Select-Object id, @{name = "Name"; Expression = { $_.type.displayName } } | format-table
        }
    }
}
else {
